<template>
    <div class="cars">
        ...
        <template v-if="carStore.list.state === 'loading'">
            <Loader />
        </template>

        <template v-if="carStore.list.state === 'loaded'">
            ...
            <template v-for="car in carStore.list.data" :key="car._id">
                <div class="car">
                    <div class="car-header">
                        {{ car.color }} {{ car.make }} {{ car.model }} - {{ car.registration }}
                    </div>
                    <div class="car-content">
                        <div>Падежна дата ГО: {{ fullDate(car.insuranceDueDate) }}</div>
                        <div>Следващ ГТП: {{ fullDate(car.motDate) }}</div>
        ...
        <template v-else>
            <div class="error">Възникна грешка при зареждането на колите</div>
        </template>
    </div>
...
<script setup>
    ...
    import { fullDate, day } from '@/utils/date'
